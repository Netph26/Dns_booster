...
if [ ! $(command -v ${_DIG}) ]; then
 printf "%b" "Dig command failed to run, " \lease install dig(dnsutils) or check " \G_EXEC & \TOM_DIG variable inside $( cd -- "$(dirname "$0")" >/dev/null 2>&1 ; pwd -P )/$(basename "$0") file.\& exit 1
fi
endscript() {
